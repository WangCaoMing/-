...
public class SettingActivity extends Activity {
    ...
    private SettingItemLayout sil_blacknumberswitch;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //初始化服务对象
        service = new Intent(SettingActivity.this,
                LanJieService.class);
        ...
        sil_blacknumberswitch = (SettingItemLayout) findViewById(R.id.layout_setting_sil_blacknumberswitch);
        ...
        boolean state_lanjieservice = CheckTools.CheckServiceState(this,
                "com.wangmeng.phonedefender.service.LanJieService"); //获取电话拦截服务的当前状态
        sil_blacknumberswitch.SetState(state_lanjieservice); //将状态值设置给组件
        ...
        sil_displaylocation.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (state) {
                    ...
                    sprefs.edit().putBoolean("callstateservice_state", true)
                            .commit();
                } else {
                    sil_displaylocation.SetState(state);
                    stopService(service); // 关闭电话状态监听服务
                    sprefs.edit().putBoolean("callstateservice_state", false)
                            .commit();
                }
            }
        });
        ...
        sil_locationsite.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                startActivity(new Intent(SettingActivity.this,
                        SetLocationSiteActivity.class));
            }
        });

        sil_blacknumberswitch.setOnClickListener(new OnClickListener() {

            @Override
            public void onClick(View v) {
                // 获取组件现在的值(点击后, 在组件内部已经实现了状态的改变和设置)
                boolean state = sil_blacknumberswitch.GetState();
                state = !state;
                if (state) {
                    // 开启黑名单拦截服务
                    System.out.println("黑名单拦截服务开启");
                    startService(service);
                } else {
                    // 关闭黑名单拦截服务
                    System.out.println("黑名单拦截服务关闭");
                    stopService(service);

                }
                sil_blacknumberswitch.SetState(state);
